...
//    @PreAuthorize("hasAuthority('pms:member:read')")
    ...
    @ApiOperation(value = "根据用户名密码查找用户")
    @RequestMapping(value = "/selectByNameAndPassWord", method = RequestMethod.POST)
    @ResponseBody
    public Object selectByNameAndPassWord(@RequestBody UmsMember umsMember) {
        UmsMember umsMember1 = umsMemberService.selectByNameAndPassWord(umsMember.getUsername(),umsMember.getPassword());
        if (umsMember1 !=null) {
